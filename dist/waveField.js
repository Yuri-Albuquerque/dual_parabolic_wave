...
exports.WaveField = void 0;
class WaveField {
    constructor(config, waveParams, majorParabola, minorParabola, focusPoint) {
        this.config = config;
    ...
    getGridCoordinates(gridI, gridJ) {
        const { gridSize, xRange, yRange } = this.config;
        const dx = (xRange[1] - xRange[0]) / (gridSize - 1);
        const dy = (yRange[1] - yRange[0]) / (gridSize - 1);
        return {
    ...
    calculateSourceAmplitude(point, time) {
        const distance = Math.sqrt(Math.pow(point.x - this.focusPoint.x, 2) +
